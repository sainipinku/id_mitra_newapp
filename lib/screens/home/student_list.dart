...
// ─────────────────────────────────────────────────────────────
// SELECTION TOOLBAR  (shared across all 3 tabs)
// ─────────────────────────────────────────────────────────────
...
class _StudentsTabState extends State<_StudentsTab> {
  ...
  // ── FIX: Initial fetch ho gaya ya nahi, ye track karne ke liye flag
  ...
  void initState() {
    ...
    //  Initial fetch with schoolId
    cubit.fetchStudents(search: '', schoolId: widget.schoolId);

    // ── FIX: Initial fetch trigger hone ke baad flag set karo
    // taaki shimmer dikhe na ki "no_data" image
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Column(
        children: [
          // ── Banner: original simple version, no loading-based hiding
              ...
              child: Padding(
                ...
                child: Column(
                  children: [
                    ...
                    // Selection toolbar — shown below search bar
                    ...
                    Expanded(
                      child: BlocBuilder<StudentsCubit, StudentsState>(
                        builder: (context, state) {
                          ...
                          // ─────────────────────────────────────────────
                          // ── FIX: PROPER LOADING STATE PRIORITY ──
                          // Pehle loading/syncing check karo,
                          // tab hi empty check karo, taaki "no_data"
                          // race condition me na dikhe
                          // ─────────────────────────────────────────────

                          // Initial fetch trigger hone se pehle shimmer dikhao
                          ...
                          // Pagination loading bhi check karo - list khali ho ja sakti hai briefly
                          if (state.isPaginationLoading &&
                              state.studentsList.isEmpty) {
                            return const ShimmerList(expanded: false);
                          }

                          // Sab loading complete - ab agar list khali hai
                          // toh actually "no data" hai
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: BlocListener<CorrectionCubit, CorrectionState>(
        ...
        child: Column(
          children: [
            // ── FIX: Banner ko loading ke time hide karo
            BlocBuilder<CorrectionCubit, CorrectionState>(
              buildWhen: (p, c) =>
              p.studentsTotal != c.studentsTotal ||
                  p.studentsLoading != c.studentsLoading,
              builder: (_, s) {
                if (s.studentsLoading && s.students.isEmpty) {
...
class _CorrectionCardState extends State<_CorrectionCard> {
  ...
  Future<void> _uploadImage(String path) async {
    ...
    try {
      ...
      // CorrectionStudentData → StudentDetailsData (upload ke liye)
      final studentData = StudentDetailsData(
        id: s.id,
        uuid: s.uuid,
        schoolId: s.schoolId,
        name: s.name,
      ...
      // StudentsCubit ka uploadStudentImage use karo — offline support hai
      ...
      // Updated photo dikhao — pehle cubit state check karo, phir local DB
      final uuid = s.uuid ?? '';
      ...
      try {
        updated = context
            ...
            .state
            .studentsList
            .firstWhere((st) => st.uuid == uuid);
      } catch (_) {}

      updated ??= await StudentLocalDS().getStudentByUuid(uuid);

      if (updated != null && mounted) {
        setState(() {
          if (updated!.isPhotoPendingSync &&
              updated.offlinePhotoPath != null) {
            // Offline: local file dikhao
            _currentPhotoUrl = updated.offlinePhotoPath;
          } else if (updated.profilePhotoUrl != null &&
              updated.profilePhotoUrl!.isNotEmpty) {
            // Online: server URL dikhao
            _currentPhotoUrl = updated.profilePhotoUrl;
          }
        });
      }
    } catch (e) {
                ...
                child: Column(
                  children: [
                    if (state.selectedOrderUuids.isNotEmpty)
                      _SelectionToolbar(
                        selectedCount:
                        state.selectedOrderUuids.length,
                        onSelectAll: () => context
                            .read<OrdersCubit>()
                            .selectAllOrders(),
                        onClear: () => context
                        ...
                        onAction: () => _showChangeStatusDialog(
                          context,
                          // map selected IDs back to real UUIDs for the API
                          state.ordersList
                              .where((o) => state.selectedOrderUuids
                      ...
                      child: ListView.builder(
                        ...
                        itemBuilder: (_, i) {
                          if (i < state.ordersList.length) {
                            ...
                            // use id-based key so empty/duplicate UUIDs
                            // don't cause the whole list to appear selected
